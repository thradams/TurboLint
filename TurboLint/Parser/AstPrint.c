	...
	switch (p->type)
	{
		...
		CASE(TUnaryExpressionOperator) :
		{
			...
			if (pTUnaryExpressionOperator->token == TK_SIZEOF)
			{
				...
				{
					fprintf(fp, "\"type\":{");
					b = TTypeQualifier_Print(&pTUnaryExpressionOperator->TypeName.Specifiers.TypeQualifiers, false, fp);
		...
		CASE(TCastExpressionType) :
		{
			...
			fprintf(fp, ",\"cast\":{");
			b = TTypeQualifier_Print(&pCastExpressionType->TypeName.Specifiers.TypeQualifiers, false, fp);
...
bool TDeclarator_Print(TDeclarator* p, bool b, FILE* fp)
{
	...
	if (p->pDeclaratorOpt)
	{
		...
		fprintf(fp, "\"declarator\":");
		b = TDeclarator_Print(p->pDeclaratorOpt, b, fp);
	}

	b = TTypeQualifier_Print(&p->Qualifiers, b, fp);
	...
	if (p->pParametersOpt != NULL)
	{
		...
		b = TParameterList_Print(p->pParametersOpt, b, fp);
	}
	...
	if (p->pExpression != NULL)
	{
		...
		//fprintf(fp, "\"array-expression\":");
		b = TExpression_Print(p->pExpression, "", b, fp);
	}
...
{
	fprintf(fp, "{");
	b = TTypeQualifier_Print(&p->Qualifier, false, fp);
...
{
	fprintf(fp, "{");
	TTypeQualifier_Print(&pPointer->Qualifier, false, fp);
	fprintf(fp, "}");
	return true;
}
...
bool TDeclarationSpecifiers_Print(TDeclarationSpecifiers* pDeclarationSpecifiers, bool b, FILE* fp)
{
	...
	b = TTypeQualifier_Print(&pDeclarationSpecifiers->TypeQualifiers, b, fp);
...
bool TDeclaration_Print(TDeclaration* p, bool b, FILE* fp)
{
	...
	fprintf(fp, "\"declarators\":");
	b = TDeclaratorList_Print(&p->Declarators, b, fp);
